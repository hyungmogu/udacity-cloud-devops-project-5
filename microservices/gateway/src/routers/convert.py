...
from config import API_MAX_REQUESTS_PER_DAY, API_SECONDS_IN_DAY, SERVER_JPG_HOST, SERVER_PNG_HOST, SERVER_WEBP_HOST
...
async def convert_to_jpg(image: UploadFile, request: Request):
    ...
    outbound_url = "{}{}".format(SERVER_JPG_HOST, urlparse(request.url).path)
...
async def convert_to_png(image: UploadFile, request: Request):
    outbound_url = "{}{}".format(SERVER_PNG_HOST, urlparse(request.url).path)
    async_client = httpx_client_wrapper()
    res = await async_client.post(outbound_url, files={'file': image.file})
    result = res.text
    return result
...
async def convert_to_webp(image: UploadFile, request: Request):
    outbound_url = "{}{}".format(SERVER_WEBP_HOST, urlparse(request.url).path)
    async_client = httpx_client_wrapper()
    res = await async_client.post(outbound_url, files={'file': image.file})
    result = res.text
    return result